...
class CustomPlayer:
    ...
    def __init__(self, search_depth=4, eval_fn=OpenMoveEvalFn(), use_open_moves=False):
    ...
    def move(self, game, time_left):
        ...
        # hog the middle square
        if self.use_open_moves and not self.moved:
            if game.is_spot_open(game.height//2, game.width//2):
                return game.height//2, game.width//2
            elif game.is_spot_open((game.height//2)+1, game.width//2):
                return (game.height//2)+1, game.width // 2

#         # move is being called, obviously my turn
...
#         best_move, utility = alphabeta(self, game, time_left, depth=self.search_depth, alpha=float("-inf"), beta=float("inf"), my_turn=True)
        ...
        if game.move_count < 25:
            best_move, utility = alphabeta(self, game, time_left, depth=4,
                                                      alpha=float("-inf"), beta=float("inf"))
        else:
            best_move, utility = alphabeta(self, game, time_left, depth=10,
                                                      alpha=float("-inf"), beta=float("inf"))
...
def alphabeta(player, game, time_left, depth, alpha, beta, my_turn=True):
    ...
    def ab_max_value(player, game, time_left, depth, alpha, beta, my_turn=True):
        ...
        try:
            for move in actions:
                new_board, is_over, winner = game.forecast_move(move)
                m, score = ab_min_value(player, new_board, time_left, depth - 1, alpha, beta, False)
                if score > value:
                    value = score
                    best_move = move
                    # print("Scored move {} val: {}".format(m, score))
    ...
    def ab_min_value(player, game, time_left, depth, alpha, beta, my_turn=False):

        if time_left() < player.time_out_buffer:
            raise TimeoutError()

        if depth == 0 or len(game.get_player_moves(player)) == 0:  # not sure len(..) makes sense here
            score = player.utility(game, player)
            position = game.get_player_position(player) if my_turn else game.get_opponent_position(player)
            return position, score
        ...
        try:
            for move in actions:
                new_board, is_over, winner = game.forecast_move(move)
                m, score = ab_max_value(player, new_board, time_left, depth - 1, alpha, beta, True)  # change from f to t
                if score < value:
                    value = score
                    best_move = move
    ...
    start_time = time.time()
    if my_turn:
        m, score = ab_max_value(player, game, time_left, depth, alpha, beta, True)
    else:
        m, score = ab_min_value(player, game, time_left, depth, alpha, beta, False)
    end_time = time.time()
    print("Time to move for alphabeta: {}".format(end_time - start_time))
