...
DATACOL_ADJUSTED_CLOSE = 'Adj Close'
DATACOL_VOLUME = 'Volume'
...
def get_data_by_column(symbols, sd, ed, column_name=DATACOL_ADJUSTED_CLOSE):
    # Read in adjusted closing prices for given symbols, date range
    dates = pd.date_range(sd, ed)
    prices_all = get_data(symbols, dates, True, column_name)  # automatically adds SPY
...
def shares_to_orderbook(df_shares, symbol):
    """
    Convert trades into this output to feed to portvals:
        Date,Symbol,Order,Shares
    """
    order_book = df_shares.copy()
    order_book['Symbol'] = symbol
    order_book['Order'] = 0
    order_book['Date'] = 0

    for index, data in order_book.iterrows():
        order_book.loc[index, 'Date'] = index
        shares = data[order_book.columns.get_loc('Shares')]
        if shares > 0:
            order_book.loc[index, 'Order'] = 'BUY'
        elif shares < 0:
            order_book.loc[index, 'Order'] = 'SELL'
        order_book.loc[index, 'Shares'] = abs(shares)
    # Chop zeros out of order book, don't recommend
    # order_book = order_book[order_book.Order != 0]
    return order_book
...
def compute_portvals(orders_df, start_val=1000000, commission=9.95, impact=0.005):
    '''
    :param orders_df: File or name of a file from which to read orders, and
    ...
    # if already datafile, use it, other try to read.....
    if type(orders_df) is not pd.core.frame.DataFrame:
        orders_df = __read_orders_file(orders_df)
    # todo: sort orders
    start_date = orders_df.index.min()
    end_date = orders_df.index.max()
    unique_symbols = orders_df['Symbol'].unique()

    prices, prices_SPY = get_data_by_column(unique_symbols.tolist(), start_date, end_date, DATACOL_ADJUSTED_CLOSE)
...
def trades2portvals(trade_symbol, df_trades, portval_sv, portval_commission=0.0, portval_impact=0.0):
    df_orders = shares_to_orderbook(df_trades, trade_symbol)
    tmp_portvals = compute_portvals(df_orders, portval_sv, portval_commission, portval_impact)
    tmp_portvals_series = pd.Series(tmp_portvals['VALUE'], index=tmp_portvals.index)
    print compute_stats(tmp_portvals_series)
    tmp_norm_portvals = tmp_portvals_series / tmp_portvals_series[0]
    return tmp_norm_portvals
...
def test_orders_short():
    compute_portvals(orders_df="orders/orders-short.csv", start_val=1000000)
